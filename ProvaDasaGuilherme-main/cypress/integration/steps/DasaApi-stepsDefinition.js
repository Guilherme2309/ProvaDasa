...
import searchByMovie from '../pages/search-by-Filmes-page'
import searchById from '../pages/search-by-id-page'

// @search_by_movie
Given(/^que eu realizo uma requisição para consultar um  "([^"]*)"$/, function(filme) {
    GetAPI.GetMovie(filme, Cypress.env('token')).should((response) => {
        this.response_movie = response
    })
});

Then(/^o filme sera validado quando pesquisado no SearchByMovie: "([^"]*)"$/, function(titulo) {
